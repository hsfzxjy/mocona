...
@pytest.mark.parametrize('op', tenary_ops)
def test_tenary_ops(op):
    a = _Dummy(1)
    b = _Dummy(2)
    c = _Dummy(3)
...
@pytest.mark.parametrize('op', inplace_binary_ops)
def test_inplace_binary_ops_immutable(op):
    a = _Dummy(1)
    b = _Dummy(2)
    a.immutable = True
    vara = S._varfor(V.a['w'])
...
@pytest.mark.parametrize('op', inplace_binary_ops)
def test_inplace_binary_ops_mutable(op):
    a = _Dummy(1)
    b = _Dummy(2)
    a.immutable = False
    vara = S._varfor(V.a['w'])
